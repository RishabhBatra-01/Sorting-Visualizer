...
def drawData(data, colorArray):
    ...
    c_height = 380
    c_width = 600
    x_width = c_width / (len(data) + 1)             #width of bar accoring the array
    offset =  30                                    #that bar don't start with the border
    spacing = 10                                    #spacing between the bar
    ...
    for i, height in enumerate(normalizedData):
        ...
        canvas.create_text(x0 + 2, y0, anchor=SW, text=str(data[i]))  #the actual number written on the bar
...
def Generate():
    ...
    minVal = int(minEntry.get())    #here we ask fro minium value
    maxVal = int(maxEntry.get())    #here we ask fro maximum value
    size = int(sizeEntry.get())      #here we ask fro size
    ...
    drawData(data, ['red' for x in range(len(data))])  # ['orange', 'orange' ,....] this line is going to pass color
